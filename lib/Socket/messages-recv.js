...
export const makeMessagesRecvSocket = (config) => {
    ...
    // Track repeated skmsg decrypt failures per (group, author) to trigger sender-key recovery
    const skmsgFailureCache = new NodeCache({ stdTTL: 300, useClones: false }); // 5 min window
    const SKMSG_FAILURE_THRESHOLD = 2;
    
    const recoverGroupSenderKey = async (remoteJid, author) => {
        try {
            logger.warn({ remoteJid, author }, 'repeated skmsg decrypt failure, forcing sender-key + session recovery');
            const memory = await authState.keys.get('sender-key-memory', [remoteJid]);
            if (memory[remoteJid]?.[author]) {
                const updated = { ...memory[remoteJid] };
                delete updated[author];
                await authState.keys.set({ 'sender-key-memory': { [remoteJid]: updated } });
            }
            await assertSessions([author], true);
        }
        catch (err) {
            logger.warn({ err, remoteJid, author }, 'sender-key recovery attempt failed');
        }
    };
        ...
        if (updateNode) {
            ...
            switch (opName) {
                ...
                case 'NotificationNewsletterJoin':
                    handleNewsletterJoinNotification(data);
                    break;
    ...
    /**
     * Handles NotificationNewsletterJoin, which arrives via the newer
     * xwa2_notify_* mex payload shape (data.xwa2_notify_newsletter_on_join),
     * not the legacy <mex> child / top-level "updates" array shape that
     * handleLegacyMexNewsletterNotification expects. Previously this op was
     * routed to the legacy handler, which found no "updates" key and silently
     * dropped every join notification with an "invalid mex newsletter
     * notification content" warning.
     *
     * NOTE: 'newsletter.join' is not a confirmed existing event name — verify
     * against your BaileysEventMap / Types definitions and adjust the emitted
     * event name and payload shape as needed before wiring up consumers.
     */
    const handleNewsletterJoinNotification = (data) => {
        const payload = data.xwa2_notify_newsletter_on_join;
        if (!payload) {
            logger.warn('newsletter join notification missing payload');
            return;
        }
        logger.info({ id: payload.id }, 'received newsletter join notification');
        ev.emit('newsletter.join', {
            id: payload.id,
            name: payload.thread_metadata?.name?.text,
            description: payload.thread_metadata?.description?.text,
            subscribers: payload.thread_metadata?.subscribers_count
                ? parseInt(payload.thread_metadata.subscribers_count, 10)
                : undefined,
            state: payload.state?.type,
            viewerRole: payload.viewer_metadata?.role,
            muteState: payload.viewer_metadata?.mute
        });
    };
                    ...
                    if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
                        ...
                        await retryMutex.mutex(async () => {
                            try {
                                ...
                                // --- skmsg sender-key recovery hook ---
                                const isSkmsg = encNode?.attrs?.type === 'skmsg';
                                if (isSkmsg && msg.key.remoteJid && author) {
                                    const failKey = `${msg.key.remoteJid}:${author}`;
                                    const count = (skmsgFailureCache.get(failKey) || 0) + 1;
                                    skmsgFailureCache.set(failKey, count);
                                    if (count >= SKMSG_FAILURE_THRESHOLD) {
                                        skmsgFailureCache.del(failKey);
                                        await recoverGroupSenderKey(msg.key.remoteJid, author);
                                    }
                                }
                                // --- end hook ---
