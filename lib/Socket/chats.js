...
export const makeChatsSocket = (config) => {
    ...
    let privacySettings;
    let privacySettingsFetch; // in-flight promise, dedupes concurrent callers
        ...
        if (!privacySettings || force) {
            if (!privacySettingsFetch) {
                privacySettingsFetch = (async () => {
                    ...
                    return reduceBinaryNodeToDictionary(content?.[0], 'category');
                })();
            }
            try {
    ...
    const updateProfilePicture = async (jid, content, dimensions) => {
        ...
        // Add tctoken for non-self profile picture updates
        let tcTokenContent;
        if (serverProps.profilePicPrivacyToken && targetJid) {
            const isUserJid = isPnUser(targetJid) || isLidUser(targetJid);
            if (isUserJid) {
        ...
        // Build the picture node with optional tctoken
        const pictureNode = {
            tag: 'picture',
            attrs: { type: 'image' },
            content: img
        ...
        // Add tctoken as content if available
        if (tcTokenContent?.length) {
