...
export const makeSocket = (config) => {
    ...
    const query = async (node, timeoutMs) => {
        ...
        const waitPromise = waitForMessage(msgId, timeoutMs);
        try {
            await sendNode(node)
        } catch (err) {
            // don't leave the waiter uncleaned — let it settle/clean itself,
            // but don't leak an unhandled rejection warning while we do
            waitPromise.catch(() => {});
            throw err;
        }
