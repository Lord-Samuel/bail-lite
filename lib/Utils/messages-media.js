...
import { extractImageThumbMini, generateProfilePictureMini } from './my-image.js';
...
export const extractImageThumb = async (bufferOrFilePath, width = 32) => {
    ...
    // bufferOrFilePath may be a file path string in some call sites —
    // mini-image only accepts buffers, so read the file first if needed
    const buffer = typeof bufferOrFilePath === 'string'
        ? await fs.readFile(bufferOrFilePath)
        : bufferOrFilePath;
    return extractImageThumbMini(buffer, width);
};
...
export const generateProfilePicture = async (mediaUpload, dimensions) => {
    ...
    return generateProfilePictureMini(buffer, dimensions);
};
